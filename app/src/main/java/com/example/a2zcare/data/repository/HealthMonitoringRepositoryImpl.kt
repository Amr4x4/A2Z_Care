...
class HealthMonitoringRepositoryImpl @Inject constructor(
    ...
    override suspend fun login(request: LoginRequest): Result<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.login(request)
                if (response.isSuccessful) {
                    // Note: You'll need to extract token from response headers or body
                    // This depends on how your backend returns the token
                    val token = response.headers()["Authorization"] ?: response.headers()["Token"]
                    token?.let { tokenManager.saveToken(it) }
                    Result.Success(Unit)
                } else {
    ...
    override suspend fun logout(): Result<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                ...
                if (userId != null && token != null && deviceId != null) {
                    val logoutRequest = LogoutRequest(userId, token, deviceId)
    ...
    override suspend fun updateUser(id: String, request: UpdateUserRequest): Result<String> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.updateUser(id, request)
                if (response.isSuccessful) {
    ...
    override suspend fun getUserData(userId: String): Result<String> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.getUserData(userId)
                if (response.isSuccessful) {
    ...
    override suspend fun createEmergencyContact(userId: String, request: EmergencyContactRequest): Result<String> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.createEmergencyContact(userId, request)
                if (response.isSuccessful) {
                    ...
                    Result.Error("Failed to create emergency contact: ${response.message()}")
                }
            } catch (e: Exception) {
                Result.Error("Network error: ${e.message}", e)
            }
    ...
    override suspend fun getEmergencyContacts(userId: String): Result<UserWithEmergencyContacts> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.getEmergencyContacts(userId)
                if (response.isSuccessful) {
                    ...
                    if (body != null) {
                        ...
                        Result.Error("No emergency contacts found")
                    }
                } else {
                    Result.Error("Failed to get emergency contacts: ${response.message()}")
                }
    ...
    override suspend fun connectContact(request: ConnectContactRequest): Result<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.connectContact(request)
                if (response.isSuccessful) {
    ...
    override suspend fun disconnectContact(userId: String, contactId: Int): Result<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.disconnectContact(userId, contactId)
                if (response.isSuccessful) {
    ...
    override suspend fun importSensorData(userId: String, request: SensorDataRequest): Result<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.importSensorData(userId, request)
                if (response.isSuccessful) {
    ...
    override suspend fun getCachedSensorData(userId: String): Result<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.getCachedSensorData(userId)
                if (response.isSuccessful) {
    ...
    override suspend fun predictActivity(request: ActivityPredictionRequest): Result<String> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.predictActivity(request)
                if (response.isSuccessful) {
                    ...
                    Result.Error("Activity prediction failed: ${response.message()}")
                }
            } catch (e: Exception) {
                Result.Error("Network error: ${e.message}", e)
            }
    ...
    override suspend fun getActivityDataByUser(userId: String): Result<String> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.getActivityDataByUser(userId)
                if (response.isSuccessful) {
                    ...
                    Result.Error("Failed to get activity data: ${response.message()}")
                }
            } catch (e: Exception) {
                Result.Error("Network error: ${e.message}", e)
            }
    ...
    override suspend fun getLatestActivityData(userId: String): Result<String> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.getLatestActivityData(userId)
                if (response.isSuccessful) {
                    ...
                    Result.Error("Failed to get latest activity data: ${response.message()}")
                }
            } catch (e: Exception) {
                Result.Error("Network error: ${e.message}", e)
            }
    ...
    override suspend fun sendBloodPressureAI(userId: String, batchSize: Int): Result<String> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.sendBloodPressureAI(userId, batchSize)
                if (response.isSuccessful) {
                    ...
                    Result.Error("Blood pressure AI failed: ${response.message()}")
                }
            } catch (e: Exception) {
                Result.Error("Network error: ${e.message}", e)
            }
    ...
    override suspend fun getLatestBloodPressurePrediction(userId: String): Result<String> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.getLatestBloodPressurePrediction(userId)
                if (response.isSuccessful) {
                    ...
                    Result.Error("Failed to get blood pressure prediction: ${response.message()}")
                }
            } catch (e: Exception) {
                Result.Error("Network error: ${e.message}", e)
            }
    ...
    override suspend fun sendHeartDiseaseAI(userId: String, batchSize: Int): Result<String> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.sendHeartDiseaseAI(userId, batchSize)
                if (response.isSuccessful) {
                    ...
                    Result.Error("Heart disease AI failed: ${response.message()}")
                }
            } catch (e: Exception) {
                Result.Error("Network error: ${e.message}", e)
            }
    ...
    override suspend fun getLatestHeartDiseasePrediction(userId: String): Result<String> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.getLatestHeartDiseasePrediction(userId)
                if (response.isSuccessful) {
                    ...
                    Result.Error("Failed to get heart disease prediction: ${response.message()}")
                }
            } catch (e: Exception) {
                Result.Error("Network error: ${e.message}", e)
            }
    ...
    override suspend fun sendHeartRateAI(userId: String, batchSize: Int): Result<String> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.sendHeartRateAI(userId, batchSize)
                if (response.isSuccessful) {
                    ...
                    Result.Error("Heart rate AI failed: ${response.message()}")
                }
            } catch (e: Exception) {
                Result.Error("Network error: ${e.message}", e)
            }
    ...
    override suspend fun getLatestHeartRateCalculation(userId: String): Result<String> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.getLatestHeartRateCalculation(userId)
                if (response.isSuccessful) {
                    ...
                    Result.Error("Failed to get heart rate calculation: ${response.message()}")
                }
            } catch (e: Exception) {
                Result.Error("Network error: ${e.message}", e)
            }
    ...
    override suspend fun sendSMS(request: SendSMSRequest): Result<Unit> {
        return withContext(Dispatchers.IO) {
            try {
                val response = apiService.sendSMSTwilio(request)
                if (response.isSuccessful) {
